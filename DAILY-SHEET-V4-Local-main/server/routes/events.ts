...
export function registerEventRoutes(app: Express, upload: multer.Multer) {
  ...
  app.patch("/api/events/:id", isAuthenticated, requireRole("owner", "manager", "admin"), async (req: any, res) => {
    try {
      ...
      const { venueForAllDays, ...data } = req.body as Partial<{ name: string; color: string; notes: string; startDate: string; endDate: string; venueId: number | null; projectId: number | null; venueForAllDays: boolean }>;
